...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from16, paymaster, paymasterInput } = transaction;
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce: nonce2, to, from: from16, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce: nonce2, to, from: from16, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from16),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from16, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from16,
    to ?? "0x",
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload3(from13(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload3(from13(envelope)),
  ...
  async verifyHash(client, parameters) {
    ...
    if (envelope) {
      if ((envelope == null ? void 0 : envelope.type) === "keychain" && mode2 === "allowAccessKey") {
        const accessKeyAddress = fromPublicKey(from3(envelope.inner.publicKey));
...
//# sourceMappingURL=chunk-HILM3TSW.js.map
